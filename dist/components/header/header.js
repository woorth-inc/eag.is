...
`,v=r.div`
  ${({media:e})=>e.isTablet?"":`
    ...
    `}
`;export default y(({items:e,media:s,refs:n})=>{const[p,l]=c(e.findIndex(({active:t})=>t)),[m,d]=c(performance.now()),g=f((t,i)=>{performance.now()-m<1e3||(n[t].current?.getAttribute("ignore-animation")==="true"?n[t].current?.setAttribute("ignore-animation","false"):i(),l(t),d(performance.now()))},[]);return o.createElement(v,{media:s},o.createElement(w,{src:`/build/${k}`}),!s.isTablet&&o.createElement(x,{items:e.length},e.map(({delay:t,label:i,onClick:u},a)=>o.createElement(b,{key:a,delay:+t,active:a===p,ref:n[a],onClick:()=>g(a,u)},i))))});
