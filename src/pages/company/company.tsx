...
const LeftWrapper = styled.div.attrs((props: Partial<Attrs>) => ({
  media: props.media || defaultMedia
}))`
  & > p {
    margin: 0;
...
const RightWrapper = styled.div.attrs((props: Partial<Attrs>) => ({
  media: props.media || defaultMedia
}))`
...
const handleClickCorporateLink = () => {
  open('https://woorth.io/')
}

const handleClickContactLink = () => {
  open('https://notionforms.io/forms/eagis-contact-form')
}
...
const Content = memo(({
  ...
  <>
    ...
    {!withoutLink && (
      <LinkWrapper>
        <Link onClick={handleClickCorporateLink}>コーポレートサイトへ</Link>
        <Link onClick={handleClickContactLink}>お問い合わせ</Link>
      </LinkWrapper>
    )}
  </>
))
      ...
      {media.isTablet ? (
        <>
          ...
          <LeftWrapper media={media}>
            ...
            <StyledButton action={handleClickCorporateLink}>コーポレートサイト</StyledButton>
            <StyledButton action={handleClickContactLink}>お問い合わせ</StyledButton>
          </LeftWrapper>
        </>
      ) : (
        <>
