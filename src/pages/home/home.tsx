...
const CardWrapper = styled.div.attrs((props: Partial<Attrs>) => ({
  media: props.media || defaultMedia,
}))`
  position: relative;
...
const StyledCard = styled(Card).attrs((props: Partial<Attrs & CardProps>) => ({
	index: props.index || 0,
  ...
  left: calc(50% + ${({ index }) => ['-300px', '0px', '300px'][index]});
  ...
  animation: ${({ animation }) => `${animation}-card`} 800ms ease ${({ index, firstRender }) => (firstRender ? 900 : 50) + index * 120}ms forwards;
`

const CardForTablet = styled.img.attrs((props: Partial<Attrs>) => ({
  animation: props.animation,
  index: props.index || 0,
  firstRender: props.firstRender,
}))`
  ...
  transform: translateX(-50%);
  left: calc(50% + ${({ index }) => [-170, 0, 170][index]}px);
  z-index: calc(3 - ${({ index }) => index});
...
const Title = styled.p.attrs((props: Partial<Attrs>) => ({
  animation: props.animation,
  index: props.index || 0,
  firstRender: props.firstRender,
}))`
  opacity: ${({ animation }) => animation === 'fadein' ? 0 : 1};
  animation: ${({ animation }) => `${animation}-title`} 800ms ease ${({ index, firstRender }) => (firstRender ? 900 : 50) + index * 120}ms forwards;
`

const TitleWrapper = styled.div.attrs((props: Partial<Attrs>) => ({
  media: props.media || defaultMedia,
}))`
  font-weight: 900;
  font-size: 30px;
...
const StyledButton = styled(Button).attrs((props: Partial<Attrs>) => ({
  media: props.media || defaultMedia,
  animation: props.animation,
  firstRender: props.firstRender,
  style: props.style,
...
const Description = styled.div.attrs((props: Partial<Attrs>) => ({
  media: props.media || defaultMedia,
  animation: props.animation,
  firstRender: props.firstRender,
}))`
